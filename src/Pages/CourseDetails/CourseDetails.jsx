  ...
  return (
    <div ref={ref} className='px-4'>
      ...
      <div  className="flex justify-between mb-4">
        <h1 className="md:text-3xl text-lg font-semibold text-primary">{course.name}</h1>
        <div>
          
        <ReactToPdf x={150} y={20}targetRef={ref} filename={`${course.name}.pdf`} options={options}>
        {({ toPdf, targetRef }) => (
          <button onClick={toPdf}
          ref={targetRef} className="btn btn-outline xs:w-full  mr-4 btn-primary btn-sm font-bold shadow-md">
          Download PDF
        </button>
        )}
      </ReactToPdf>
          ...
          <Link to={`/courses/checkout/${course._id}`}>
            <button className="btn btn-outline xs:w-full btn-primary btn-sm font-bold shadow-md">
              Chekout
            </button>
          </Link>
        </div>
      ...
      {/* write review */}
      <div>
        <h1 className="text-3xl text-primary my-8 font-semibold">
          Write a Review
        </h1>
